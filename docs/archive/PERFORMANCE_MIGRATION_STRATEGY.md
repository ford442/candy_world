...
- _Comment for Agent:_ "I have left `logic.js` in place so you can easily add new game mechanics there later."
...
2.  **Phase 2 (Partial TS):** Gravity math is buggy. Extract _just_ the math to `celestial-math.ts`. `celestial.js` imports it.
3.  **Phase 3 (Partial WASM):** We have 10,000 bodies. Move _just_ the loop to `assembly/celestial.ts`.
...
## Tier parity (correctness gate)

The runtime loader (`src/utils/wasm-loader-core.ts` and friends) picks JS → AssemblyScript → C++ per function. **Performance is not enough** — migrated hot loops must agree within documented epsilon.

- **Harness:** `tests/tier-parity.mjs` (+ `tests/tier-parity-references.mjs`)
- **Docs:** [TIER_PARITY.md](../TIER_PARITY.md) — canonical tier per function, epsilon table, CI behavior
- **CI:** `pnpm run test:integration` runs parity after `test:wasm`; C++ tier is best-effort (skipped without Emscripten build)
- **When to extend:** Any new 15% slice that adds a second implementation (e.g. `batchShiver` in C++ _and_ a JS fallback) should add golden vectors here before merge
