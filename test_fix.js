...
content = content.replace(/this\._lastCameraPos\.copy\(camera\.position\);\n\n {8}updateBiomeChannelBindings\(audioState, getDayNightBias\(time % CYCLE_DURATION\), this\._lastCameraPos\);/g, 'updateBiomeChannelBindings(audioState, getDayNightBias(time % CYCLE_DURATION), camera.position);');
...
content = content.replace(/ {4}private _lastCameraPos = new THREE\.Vector3\(\);\n/g, '');
