...
// Buffer Layout: [x, y, z, life, vx, vy, vz, speed] (8 floats per particle) // 4 bytes per float * 8 floats = 32 bytes per particle

export function initParticles(ptr: usize, count: i32): void { for (let i = 0; i < count; i++) { let offset = ptr + (<usize>i * 32);
...
export function updateParticles(ptr: usize, count: i32, dt: f32): void { for (let i = 0; i < count; i++) { let offset = ptr + (<usize>i * 32);
...
export function checkCollision(ptr: usize, count: i32, playerX: f32, playerZ: f32, radius: f32): void { let rSq = radius * radius;
...
for (let i = 0; i < count; i++) {
    ...
    if (py < 0.0) {
        ...
        store<f32>(offset + 20, -vy * 0.5); // vy bounce with damping
    }

    // Simple player push (cylinder check)
    let dx = px - playerX;
    let dz = pz - playerZ;
