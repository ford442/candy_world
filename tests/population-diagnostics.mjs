...
async function run() {
    ...
    try {
        ...
        await page.goto('http://localhost:4173/?boot=explore', {
        ...
        while (Date.now() - startPoll < 90000) {
            ...
            if (report.spawnReport && report.spawnReport.attempted > 0) {
                lastReport = report;
                console.log(
                    `[Poll] attempted=${report.spawnReport.attempted} succeeded=${report.spawnReport.succeeded} failed=${report.spawnReport.failed}`
                );
            }
            await new Promise((r) => setTimeout(r, 2000));
        }
