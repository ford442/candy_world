    ...
    try {
        ...
        page.on('console', (msg) => {
            ...
            if (msg.type() === 'error') {
                ...
                if (
                    text.includes('WebGPU Device Lost') ||
                    text.includes('Device was destroyed') ||
                    text.includes(
                        'Failed to load resource: the server responded with a status of 404'
                    ) ||
                    text.includes('candy_native_st.js not found. Fallback?') ||
                    text.includes('candy_native.js not found. Fallback?') ||
                    text.includes(
                        'Failed to fetch dynamically imported module: http://localhost:4173/chunks/candy_native'
                    ) ||
                    text.includes('Missing required export') ||
                    text.includes('AS init attempt')
                ) {
        ...
        if (!gpuContext) {
            ...
            if (!ok) {
                console.error(
                    `[CONSOLE ERROR] Incomplete GPU context: ${JSON.stringify(gpuContext)}`
                );
                hasError = true;
            } else {
                console.log(
        ...
        try {
            ...
            await page.waitForSelector('#candy-loading-overlay', {
                state: 'hidden',
                timeout: 5000,
            });
        ...
        if (IS_FULL_BOOT) {
            ...
            if (!hasError) {
                ...
                try {
                    ...
                    await page.waitForTimeout(4000);
                    await page.waitForFunction(() => window.__worldHealth !== undefined, {
                        timeout: 60000,
                    });
                    ...
                    if (msg.includes('Target crashed') || msg.includes('Session closed')) {
                        ...
                        console.error(
                            '   This often happens in headless environments without stable WebGPU support.'
                        );
                        console.error(
                            '   Run FULL_BOOT=1 on a machine with a real GPU for reliable results.'
                        );
                        hasError = true;
                    } else {
            ...
            if (!hasError) {
                    ...
                    result = await page.evaluate(() => {
                        ...
                        if (!health) {
                            ...
                            if (health.succeeded < 1000) {
                                errors.push(
                                    `Expected >=1000 succeeded spawns, got ${health.succeeded}`
                                );
                            }
                            if (health.sceneObjects.animatedFoliage < 50) {
                                errors.push(
                                    `Expected animatedFoliage >= 50, got ${health.sceneObjects.animatedFoliage}`
                                );
                            }
                            if (health.batchers.totalInstances < 100) {
                                errors.push(
                                    `Expected batcher instances >= 100, got ${health.batchers.totalInstances}`
                                );
                            }
                        }
                        ...
                        if (!spawn) {
                            ...
                            errors.push(
                                `Expected 0 spawn failures, got ${spawn.failed} (attempted ${spawn.attempted})`
                            );
                        }
                        ...
                        if (!game || !game.animatedFoliage) {
                            ...
                            errors.push(
                                `Expected game.animatedFoliage.length >= 50, got ${game.animatedFoliage.length}`
                            );
                        }
                ...
                if (result) {
                    if (result.errors.length > 0) {
                        ...
                        console.log(
                            `   spawned: ${result.health.succeeded}/${result.health.attempted}`
                        );
                        console.log(`   failed: ${result.health.failed}`);
                        console.log(
                            `   animatedFoliage: ${result.health.sceneObjects.animatedFoliage}`
                        );
                        console.log(
                            `   batcherInstances: ${result.health.batchers.totalInstances}`
                        );
                    }
                }
            }
            ...
            try {
                ...
                if (health) {
                    console.log(
                        `[WorldHealth] mode=${health.mode} spawned=${health.succeeded}/${health.attempted} failed=${health.failed} foliage=${health.sceneObjects?.animatedFoliage} batchers=${health.batchers?.totalInstances}`
                    );
                    if (!health.healthy) {
                ...
                console.log(
                    'ℹ World health report not available within timeout (background tasks still running or CORE mode)'
                );
            }
        }
        ...
        try {
            ...
            if (errorTiming.lastWarmupMaterial) {
                console.log(
                    `[DIAGNOSTIC] Last warmup material before error: ${errorTiming.lastWarmupMaterial}`
                );
            }
        } catch (e) {
            const msg = e.message || '';
