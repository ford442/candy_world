...
- [x] **#1448 Unify WebGPU device & renderer context (single-device architecture)** — `priority-high`. **Status: Implemented ✅**.
...
- [x] **#1452 Living-World fauna flocks + optional GPU/C++ boids scale-up** — Content. **Status: Implemented ✅**.
  * Implementation Details: Replicated AssemblyScript boids logic into `emscripten/boids.cpp` with SIMD/optimization potential, wired it into `exports.txt` and the WASM loader (`wasmUpdateBoids_c`), and upgraded `boids-bridge.ts` to prefer this native C++ path when available.
...
**Next Steps:** Proceed to tackle #1452 Living-World fauna flocks now that the WebGPU device foundation is solid and guaranteed to not exceed context allocations.
