...
const replace = `        // ⚡ OPTIMIZATION: Single pass over audio channels for volume accumulations
        MRState.globalShimmerAccum = 0.0;
        MRState.globalHueShiftAccum = 0.0;
        MRState.gemCanopyShimmerAccum = 0.0;
        MRState.gemCanopyHueShiftAccum = 0.0;
        ...
        for (let i = 0; i < channels.length; i++) {
            ...
            if (MRState.skyIslandsHueShiftCh.includes(i)) MRState.skyIslandsHueShiftAccum += vol;
            ...
            if (MRState.sugarCavesHueShiftCh.includes(i)) MRState.sugarCavesHueShiftAccum += vol;
...
if (content.includes(search)) {
    ...
    console.log("Successfully patched music reactivity");
} else {
    console.log("Could not find music reactivity search block");
}
