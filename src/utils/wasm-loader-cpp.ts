...
export let cppValueNoise2DSimd4: ((xPtr: number, yPtr: number, outPtr: number) => void) | null =
    null;
export let cppFbm2DSimd4:
    ((xPtr: number, yPtr: number, octaves: number, outPtr: number) => void) | null = null;
export let cppBatchGroundHeightSimd:
    ((positionsPtr: number, count: number, outputPtr: number) => void) | null = null;
export let cppBatchValueNoiseOmp:
    ((xPtr: number, yPtr: number, count: number, outPtr: number) => void) | null = null;
export let cppBatchFbmOmp:
    ((xPtr: number, yPtr: number, count: number, octaves: number, outPtr: number) => void) | null =
    null;
export let cppBatchDistSq3DOmp:
    | ((
          axPtr: number,
          ayPtr: number,
          azPtr: number,
...
export let cppGetUnifiedGroundHeight: ((x: number, z: number, nowMs: number) => number) | null =
    null;
export let cppBatchUnifiedGroundHeight:
...
export let cppAddGroundPlatform:
...
export let cppBatchFloatSimd:
    ...
    | null = null;
export let cppBatchCloudBobSimd:
    | ((
          inputPtr: number,
          count: number,
          time: number,
    ...
    | null = null;
export let cppBatchVineSwaySimd:
    | ((
          inputPtr: number,
          count: number,
          time: number,
    ...
    | null = null;
export let cppBatchGeyserEruptC:
    | ((particlesPtr: number, count: number, time: number, kick: number, outputPtr: number) => void)
    | null = null;
export let cppBatchRetriggerSimd:
    | ((
          inputPtr: number,
          count: number,
          time: number,
...
export function initCppFunctions(): void {
    ...
    log.info('WASM', 'C++ Physics & Math functions mapped successfully');
}

export function setEmscriptenMemory(val: ArrayBuffer | null) {
    emscriptenMemory = val;
}
