    ...
    const emccPromise = (async () => {
        try {
            ...
            try {
                ...
                if (!wasmCheck.ok) {
                    throw new Error('WASM not at production path');
                }
            } catch (headError) {
                // Try local path
                locatePrefix = '';
                ...
                try {
                    const localCheck = await fetch(wasmPath, { method: 'HEAD' });
                    if (!localCheck.ok) {
                        console.log('[WASMOrchestrator] candy_native.wasm not found, skipping EMCC module');
                        return null;
                    }
                } catch (localError) {
                    console.log('[WASMOrchestrator] candy_native.wasm not available, skipping EMCC module');
                    return null;
                }
            }

            // Import the generated JS loader
            // Note: We use a dynamic import to load the file generated by Emscripten
            let createCandyNative;
            try {
                const { default: creator } = await import(/* @vite-ignore */ `/candy-world/candy_native.js?v=${Date.now()}`);
                createCandyNative = creator;
            } catch (prodError) {
                console.log('[WASMOrchestrator] Production path failed, trying local...');
                try {
                    const { default: creator } = await import(/* @vite-ignore */ `/candy_native.js?v=${Date.now()}`);
                    createCandyNative = creator;
                    locatePrefix = '';
                } catch (localError) {
