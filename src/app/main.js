...
function safeAddFoliage(obj, isObstacle = false, obstacleRadius = 1.0) {
    ...
    if (isObstacle) obstacles.push({ position: obj.position.clone(), radius: obstacleRadius });
}
...
if (musicUpload) {
    musicUpload.addEventListener('change', (e) => { if (e.target.files.length > 0) audioSystem.loadModule(e.target.files[0]); });
...
document.addEventListener('keydown', (e) => { if (e.code === 'KeyN') isNight = !isNight });

const keyStates = { forward: false, backward: false, left: false, right: false, jump: false, sneak: false, sprint: false };
const onKeyDown = function (event) { if (event.ctrlKey && event.code !== 'ControlLeft' && event.code !== 'ControlRight') event.preventDefault(); switch (event.code) { case 'KeyW': break; case 'KeyA': keyStates.left = true; break; case 'KeyS': keyStates.backward = true; break; case 'KeyD': keyStates.right = true; break; case 'Space': keyStates.jump = true; break; case 'ControlLeft': case 'ControlRight': keyStates.sneak = true; event.preventDefault(); break; case 'ShiftLeft': case 'ShiftRight': keyStates.sprint = true; break; } };
const onKeyUp = function (event) { switch (event.code) { case 'KeyW': break; case 'KeyA': keyStates.left = false; break; case 'KeyS': keyStates.backward = false; break; case 'KeyD': keyStates.right = false; break; case 'Space': keyStates.jump = false; break; case 'ControlLeft': case 'ControlRight': keyStates.sneak = false; break; case 'ShiftLeft': case 'ShiftRight': keyStates.sprint = false; break; } };
const onMouseDown = function (event) { if (event.button === 2) keyStates.forward = true };
const onMouseUp = function (event) { if (event.button === 2) keyStates.forward = false };
document.addEventListener('keydown', onKeyDown);
document.addEventListener('keyup', onKeyUp);
document.addEventListener('mousedown', onMouseDown);
document.addEventListener('mouseup', onMouseUp);

const player = { velocity: new THREE.Vector3(), direction: new THREE.Vector3(), sneakSpeed: 10.0, runSpeed: 30.0, sprintSpeed: 50.0, currentSpeed: 30.0, acceleration: 20.0, gravity: 20.0, jumpStrength: 10.0, height: 1.8, radius: 0.5 };

let drivingMushroom = null; let previousCameraPosition = null;
function findNearestDrivableMushroom() { let minDist = Infinity; let nearest = null; const camPos = camera.position; animatedObjects.forEach(obj => { if (obj.type === 'mushroom' && obj.drivable) { const mPos = obj.mesh.position; const dx = camPos.x - mPos.x; const dz = camPos.z - mPos.z; const dist = Math.sqrt(dx * dx + dz * dz); if (dist < minDist) { minDist = dist; nearest = obj; } } }); return nearest; }
function startDrivingMushroom(mushroom) { drivingMushroom = mushroom; previousCameraPosition = camera.position.clone(); camera.position.copy(mushroom.mesh.position); }
function stopDrivingMushroom() { if (previousCameraPosition) { camera.position.copy(previousCameraPosition); } drivingMushroom = null; }
document.addEventListener('keydown', (event) => { if (event.code === 'KeyM') { if (drivingMushroom) stopDrivingMushroom(); else { const nearest = findNearestDrivableMushroom(); if (nearest) startDrivingMushroom(nearest); } } });

let cloudHelicopter = null; let cloudIsRaining = false; let previousCameraPositionCloud = null;
...
document.addEventListener('keydown', (event) => { if (event.code === 'KeyC') { if (cloudHelicopter) dismissCloudHelicopter(); else summonCloudHelicopter(); } if (event.code === 'KeyR' && cloudHelicopter) toggleCloudRain(); });

// Waterfall + Giant mushrooms + zone generators follow (they are copied verbatim)
...
function createGiantMushroom(x, z, scale = 8) {
    ...
    group.userData.type = 'mushroom'; animatedObjects.push(giantMushroom); animatedFoliage.push(group);
}

function createGiantRainCloud(options = {}) { const { color = 0x555555, rainIntensity = 200 } = options; const group = new THREE.Group(); const cloudGeo = new THREE.SphereGeometry(4.5, 32, 32); const cloudMat = materials.cloud.clone(); cloudMat.color.setHex(color); const cloud = new THREE.Mesh(cloudGeo, cloudMat); cloud.castShadow = true; group.add(cloud); const rainGeo = new THREE.BufferGeometry(); const positions = new Float32Array(rainIntensity * 3); for (let i = 0; i < rainIntensity; i++) { positions[i*3] = (Math.random() - 0.5) * 9.0; positions[i*3 + 1] = Math.random() * -6.0; positions[i*3 + 2] = (Math.random() - 0.5) * 9.0; } rainGeo.setAttribute('position', new THREE.BufferAttribute(positions, 3)); const rainMat = new THREE.PointsMaterial({ color: 0x87CEEB, size: 0.05 }); const rain = new THREE.Points(rainGeo, rainMat); group.add(rain); group.userData.animationType = 'rain'; return group; }
...
function spawnKingMushroomZone(cx, cz) {
    ...
    obstacles.push({ position: group.position.clone(), radius: stemR * 1.2 }); scene.add(group); animatedFoliage.push(waterfall); window.kingMushroomCap = cap; window.kingWaterfall = waterfall; const splashZone = new THREE.Object3D(); splashZone.position.set(cx, 0, cz + capR * 0.8); splashZone.userData = { animationType: 'rain' }; rainingClouds.push(splashZone);
...
async function animate() {
    ...
    if (stars.material) stars.material.opacity = dayNightFactor;
    if (audioState && isNight) { uStarPulse.value = audioState.kickTrigger; const hue = (t * 0.1 + audioState.beatPhase) % 1; uStarColor.value.setHSL(hue, 1.0, 0.8); }
    if (window.kingMushroomCap && audioState) { const kick = audioState.kickTrigger || 0; const groove = audioState.grooveAmount || 0; const targetScale = 1.0 + kick * 0.3; window.kingMushroomCap.scale.setScalar(targetScale); if (window.kingWaterfall && window.kingWaterfall.material && window.kingWaterfall.material.uSpeed) { window.kingWaterfall.material.uSpeed.value = 1.0 + groove * 5.0; } }
    updateFoliageMaterials(audioState, isNight);
    if (controls.isLocked) {
        let targetSpeed = player.runSpeed; if (keyStates.sneak) targetSpeed = player.sneakSpeed; if (keyStates.sprint) targetSpeed = player.sprintSpeed; if (player.currentSpeed < targetSpeed) player.currentSpeed += player.acceleration * delta; if (player.currentSpeed > targetSpeed) player.currentSpeed -= player.acceleration * delta; player.velocity.x -= player.velocity.x * 10.0 * delta; player.velocity.z -= player.velocity.z * 10.0 * delta; player.velocity.y -= player.gravity * delta; player.direction.z = Number(keyStates.forward) - Number(keyStates.backward); player.direction.x = Number(keyStates.right) - Number(keyStates.left); player.direction.normalize(); if (keyStates.forward || keyStates.backward) player.velocity.z -= player.direction.z * player.currentSpeed * delta; if (keyStates.left || keyStates.right) player.velocity.x -= player.direction.x * player.currentSpeed * delta; if (keyStates.jump) { if (camera.position.y <= getGroundHeight(camera.position.x, camera.position.z) + player.height + 0.5) player.velocity.y = player.jumpStrength; } if (!drivingMushroom && !cloudHelicopter) { controls.moveRight(-player.velocity.x * delta); controls.moveForward(-player.velocity.z * delta); camera.position.y += player.velocity.y * delta; const groundY = getGroundHeight(camera.position.x, camera.position.z); if (camera.position.y < groundY + player.height) { player.velocity.y = 0; camera.position.y = groundY + player.height; } } }
    ...
    rainingClouds.forEach(cloud => {
        ...
        if (cloudIsRaining || cloud.userData.animationType === 'rain') {
            ...
            for (let k = 0; k < 5; k++) {
                ...
                if (dx * dx + dz * dz < 25) {
                    if (plant.scale.y < 2.0) plant.scale.multiplyScalar(1.002);
                }
            }
