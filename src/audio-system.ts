import type { AudioContext, AudioBufferSourceNode } from 'web-audio-api';
...
const extractNote = (cell: any) => cell?.text?.match(/[A-G][#-]?\d/)?.[0];
...
export class AudioSystem {
  ...
  isReady = false;
  volume = 1.0;
  visualState: any = { beatPhase: 0, kickTrigger: 0, grooveAmount: 0, activeChannels: 0, channelData: [] };
  constructor() { this.init(); }
  async init() {
    if (!window.libopenmptReady) { console.error('libopenmptReady promise not found.'); return; }
    try { const lib = await (window as any).libopenmptReady; this.libopenmpt = lib; this.isReady = true; console.log('AudioSystem initialized.'); } catch (err) { console.error('AudioSystem init failed:', err); }
  }
