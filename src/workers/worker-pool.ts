...
export class WorkerPool {
    ...
    constructor(
        ...
        if (!this.useWorkers) {
            console.log(
                '[WorkerPool] Workers disabled or not supported - will use main thread fallback'
            );
        }
    }
    ...
    async initialize(): Promise<boolean> {
        ...
        try {
            ...
            this.isInitialized = true;
            console.log(
                `[WorkerPool] Initialized with ${this.physicsWorkers.length} physics and ${this.worldGenWorkers.length} world gen workers`
            );
            return true;
        } catch (error) {
     ...
     */
    private async retryRequest(failedWorker: PooledWorker, pending: PendingRequest): Promise<void> {
        pending.retries++;
    ...
    async batchGetGroundHeight(positions: { x: number; z: number }[]): Promise<number[]> {
        ...
        const response = await this.sendRequest<BatchGroundHeightRequest>(
            'physics',
            {
