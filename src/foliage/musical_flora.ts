...
export function createSnareTrap(options: SnareTrapOptions = {}) {
    ...
    // Hitbox
    const hitGeo = new THREE.CylinderGeometry(0.5 * scale, 0.4 * scale, 0.5 * scale);
    const hitMat = new THREE.MeshBasicMaterial({ visible: false });
    const hitMesh = new THREE.Mesh(hitGeo, hitMat);
    hitMesh.position.y = 0.25 * scale;
    group.add(hitMesh);

    group.userData.animationType = 'snareSnap';
    group.userData.type = 'snare';
    group.userData.interactionText = "Snare Snap";
...
export function createRetriggerMushroom(options: RetriggerMushroomOptions = {}) {
     const { scale = 1.0, color = 0xFF00FF } = options;
     const group = new THREE.Group();

     const hitGeo = new THREE.CylinderGeometry(0.6 * scale, 0.6 * scale, 1.0 * scale);
     const hitMat = new THREE.MeshBasicMaterial({ visible: false });
     const hitMesh = new THREE.Mesh(hitGeo, hitMat);
     hitMesh.position.y = 0.5 * scale;
     group.add(hitMesh);

     group.userData.animationType = 'retriggerBounce';
     group.userData.type = 'mushroom';
     group.userData.interactionText = 'Retrigger';
