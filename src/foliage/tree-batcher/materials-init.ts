...
export function initializeTreeBatcherMeshes(
    state: TreeBatcherState,
    ...
    const circadianDroopTrunk = vec3(
        0,
    ...
    state.trunks = new THREE.InstancedMesh(
        sharedGeometries.unitCylinder,
        trunkMat,
        state.trunkCapacity
    );
    state.trunks.instanceColor = new THREE.InstancedBufferAttribute(
        new Float32Array(state.trunkCapacity * 3),
        3
    );
    state.trunks.geometry.setAttribute('instanceColor', state.trunks.instanceColor);
    state.trunks.geometry.setAttribute(
        'instanceAnimType',
        new THREE.InstancedBufferAttribute(new Float32Array(state.trunkCapacity), 1)
    );
    state.trunks.geometry.setAttribute(
        'instanceAnimOffset',
        new THREE.InstancedBufferAttribute(new Float32Array(state.trunkCapacity), 1)
    ...
    const circadianDroopSphere = vec3(
        0,
        float(-1.0).mul(uCircadianPoseOffset).mul(positionLocal.y),
        0
    );
    ...
    state.spheres = new THREE.InstancedMesh(
        sharedGeometries.unitSphere,
        sphereMat,
        state.sphereCapacity
    );
    state.spheres.instanceColor = new THREE.InstancedBufferAttribute(
        new Float32Array(state.sphereCapacity * 3),
        3
    );
    state.spheres.geometry.setAttribute('instanceColor', state.spheres.instanceColor);
    state.spheres.geometry.setAttribute(
        'instanceAnimType',
        new THREE.InstancedBufferAttribute(new Float32Array(state.sphereCapacity), 1)
    );
    state.spheres.geometry.setAttribute(
        'instanceAnimOffset',
        new THREE.InstancedBufferAttribute(new Float32Array(state.sphereCapacity), 1)
    ...
    const circadianDroopCapsule = vec3(
        0,
        float(-1.0).mul(uCircadianPoseOffset).mul(positionLocal.y),
        0
    );
    ...
    state.capsules = new THREE.InstancedMesh(
        sharedGeometries.capsule,
        capsuleMat,
        state.capsuleCapacity
    );
    state.capsules.instanceColor = new THREE.InstancedBufferAttribute(
        new Float32Array(state.capsuleCapacity * 3),
        3
    );
    state.capsules.geometry.setAttribute('instanceColor', state.capsules.instanceColor);
    state.capsules.geometry.setAttribute(
        'instanceAnimType',
        new THREE.InstancedBufferAttribute(new Float32Array(state.capsuleCapacity), 1)
    );
    state.capsules.geometry.setAttribute(
        'instanceAnimOffset',
        new THREE.InstancedBufferAttribute(new Float32Array(state.capsuleCapacity), 1)
    ...
    state.helices.instanceColor = new THREE.InstancedBufferAttribute(
        new Float32Array(state.helixCapacity * 3),
        3
    );
    state.helices.geometry.setAttribute('instanceColor', state.helices.instanceColor);
    state.helices.geometry.setAttribute(
        'instanceAnimType',
        new THREE.InstancedBufferAttribute(new Float32Array(state.helixCapacity), 1)
    );
    state.helices.geometry.setAttribute(
        'instanceAnimOffset',
        new THREE.InstancedBufferAttribute(new Float32Array(state.helixCapacity), 1)
    ...
    state.accordionLeaves = new THREE.InstancedMesh(
        accordionLeafGeo,
        accordionLeafMat,
        state.accordionLeafCapacity
    );
    state.roses.instanceColor = new THREE.InstancedBufferAttribute(
        new Float32Array(state.roseCapacity * 3),
        3
    );
    state.accordionLeaves.instanceColor = new THREE.InstancedBufferAttribute(
        new Float32Array(state.accordionLeafCapacity * 3),
        3
    );
    state.roses.geometry.setAttribute('instanceColor', state.roses.instanceColor);
    state.roses.geometry.setAttribute(
        'instanceAnimType',
        new THREE.InstancedBufferAttribute(new Float32Array(state.roseCapacity), 1)
    );
    state.accordionLeaves.geometry.setAttribute(
        'instanceAnimType',
        new THREE.InstancedBufferAttribute(new Float32Array(state.accordionLeafCapacity), 1)
    );
    state.roses.geometry.setAttribute(
        'instanceAnimOffset',
        new THREE.InstancedBufferAttribute(new Float32Array(state.roseCapacity), 1)
