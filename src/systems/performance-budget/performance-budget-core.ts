...
export class PerformanceBudget {
    ...
    async autoDetectBudget(): Promise<'mobile' | 'desktop' | 'highEnd'> {
        ...
        try {
            ...
            if (info) {
                ...
                const integrated = /intel|uhd|iris|mali|adreno|swiftshader|llvmpipe/.test(
                    description
                );
    ...
    private handleViolation(type: BudgetType, budget: number, actual: number): void {
        ...
        // Notify listeners
        for (let i = 0; i < this.onViolationCallbacks.length; i++) {
            const cb = this.onViolationCallbacks[i];
            try {
                cb(violation);
            } catch (e) {
    ...
    applyAdaptiveReduction(type?: BudgetType): void {
        ...
        if (changed) {
            console.log(
                `[PerformanceBudget] Applied adaptive reduction for ${specificType}:`,
                this.adaptiveSettings
            );

            for (let i = 0; i < this.onAdaptiveChangeCallbacks.length; i++) {
                const cb = this.onAdaptiveChangeCallbacks[i];
                try {
