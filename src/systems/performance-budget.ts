 ...
 * - Debug overlay with real-time performance visualization
...
export class PerformanceBudget {
  ...
  private updateDebugOverlay(): void {
    ...
    const frameTime = this.currentMetrics.frameTime.toFixed(2);
    const frameTimeBudget = this.budgets.get('frameTime')?.limit ?? 16.67;
    const memory = this.currentMetrics.memory.toFixed(1);
    const memoryBudget = this.budgets.get('memory')?.limit ?? 512;
    const instances = this.currentMetrics.instances;
    const instanceBudget = this.budgets.get('instances')?.limit ?? 2000;
    const drawCalls = this.drawCallCount;
    const drawCallBudget = this.budgets.get('drawCalls')?.limit ?? 500;
    ...
    html += `
      <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #444; font-size: ${10 * this.overlayOptions.scale}px; color: #aaa;">
        <div>LOD: ${(this.adaptiveSettings.lodDistanceMultiplier * 100).toFixed(0)}% | 
             Shadows: ${(this.adaptiveSettings.shadowDistanceMultiplier * 100).toFixed(0)}% | 
             Particles: ${(this.adaptiveSettings.particleDensityMultiplier * 100).toFixed(0)}%</div>
        <div>${this.adaptiveSettings.aggressiveBatching ? '⚡ Batching ' : ''}
             ${this.adaptiveSettings.meshMerging ? '🔗 Merging ' : ''}
             ${this.adaptiveSettings.unloadDistantFoliage ? '🗑️ Unload ' : ''}</div>
      </div>
    `;
    
    // Show recent violations
    if (this.overlayOptions.showViolations && this.violations.length > 0) {
  ...
  private drawPerformanceGraph(): void {
    ...
    // Draw budget line
    const budget = this.budgets.get('frameTime')?.limit ?? 16.67;
    const maxValue = Math.max(budget * 2, ...this.frameTimeHistory);
    ...
    if (this.frameTimeHistory.length > 1) {
      ...
      for (let i = 0; i < this.frameTimeHistory.length; i++) {
        ...
        const y = height - (this.frameTimeHistory[i] / maxValue) * (height - 10);
      ...
      // Highlight over-budget areas
      ctx.fillStyle = 'rgba(255, 68, 68, 0.3)';
      for (let i = 0; i < this.frameTimeHistory.length; i++) {
        if (this.frameTimeHistory[i] > budget) {
          const x = (i / (this.historyLength - 1)) * width;
          const barWidth = width / this.historyLength;
