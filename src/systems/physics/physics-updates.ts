...
/**
 * @deprecated Use stepCharacter directly. Kept for compatibility.
 */
export function updateJSFallbackMovement(
    ...
    const intent: CharacterIntent = {
        wishDir: _scratchMoveVec.copy(_scratchTargetVel),
        moveSpeed,
        jumpPressed: keyStates.jump,
        jumpTriggered: keyStates.jump,
    };
    if (intent.wishDir.lengthSq() > 0) intent.wishDir.normalize();
    stepCharacter(delta, intent);
}
